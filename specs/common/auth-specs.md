...
| `GET`    | `/api/v1/venues/{venue_id}/staff` | Список сотрудников площадки    | Yes  | Manager/Admin venue             |
| `GET`    | `/api/v1/staff/{profile_id}`      | Профиль конкретного сотрудника | Yes  | Manager/Admin или сам сотрудник |
...
2. **POST /venues/{venue_id}/staff (Management):**
...
3. **GET /venues/{venue_id}/staff:**
   - Проверка прав доступа к venue
   - Пагинация (page, limit)
   - Возврат списка с информацией о пользователях

4. **PATCH /staff/{profile_id}:**
   - Проверка прав (менеджер/админ venue)
   - Обновление `role` и/или `is_active`
   - Возврат обновленного профиля

5. **DELETE /staff/{profile_id}:**
   - Проверка прав (менеджер/админ venue)
   - Soft-delete записи (или DELETE)
   - **Не удаляет пользователя из системы**, только отвязывает от venue
